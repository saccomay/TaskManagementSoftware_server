...
import com.thupx.tms.config.KafkaProperties;
...
class TaskManagementSoftwareServerKafkaResourceIT {
    ...
    void setup() {
        ...
        TaskManagementSoftwareServerKafkaResource kafkaResource = new TaskManagementSoftwareServerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/task-management-software-server-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/task-management-software-server-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
